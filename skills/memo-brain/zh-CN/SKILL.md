...
当 `memo embed` 检测到相似记忆时（相似度 > 0.85），**不要直接跳过或强制添加**，应该按以下流程处理：

**步骤流程：**
```bash
# 1. 检测到重复后，搜索查看现有记忆的完整内容
memo search "相关关键词" -n 5

# 2. 评估并决策：
#    - 新内容是对原有内容的细化补充 → 使用 update
#    - 多条记忆有重叠且可以整合 → 使用 merge
#    - 完全独立的新知识 → 使用 --force 强制添加

# 示例 A：更新现有记忆（补充细节）
memo update abc123 --content "原有内容 + 新的细节和补充"

# 示例 B：合并相似记忆（整合重叠内容）
memo merge id1 id2 --content "整合后的完整内容，涵盖两条记忆的要点"

# 示例 C：强制添加（确认是独立知识）
memo embed "确实是新的独立知识点..." --force
```
